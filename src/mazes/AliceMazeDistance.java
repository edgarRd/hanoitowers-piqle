...
In this version, robot can see at a distance of two squares...
...
public class AliceMazeDistance extends AliceMaze{
...
public AliceMazeDistance(int lo,int la){
	super(lo,la); 
    }
...
/** Set the initial state to any free position of the maze. */
	...
	defaultCurrentState=new AliceDistanceState(i,j,generateur.nextInt(4),this); 
...
/** Build a maze according to a two-dimensional array description */
	...
	for(int i=0;i<largeur;i++)
		...
		if((!found)&&(forme[i][j].isFree()))
			...
			defaultCurrentState=new AliceDistanceState(i,j,0,this); 
			oldState=defaultCurrentState; 
			found=true; 
		    }
    }
    public IState successorState(IState s,IAction a){
	AliceDistanceState el=(AliceDistanceState) s;
	ActionAlice al=(ActionAlice)a;
	oldState=defaultCurrentState; 
	if(al.equals(ActionAlice.FORWARD))
	    switch(el.getOrientation()){
	    case 0 : return new AliceDistanceState(el.getX()-1,el.getY(),el.getOrientation(),this); 
	    case 1 : return new AliceDistanceState(el.getX(),el.getY()+1,el.getOrientation(),this); 
	    case 2 : return new AliceDistanceState(el.getX()+1,el.getY(),el.getOrientation(),this); 
	    case 3 : return new AliceDistanceState(el.getX(),el.getY()-1,el.getOrientation(),this); 
	...
	if(al.equals(ActionAlice.RIGHT))
	    return new AliceDistanceState(el.getX(),el.getY(),(el.getOrientation()+1)%4,this); 
	if(al.equals(ActionAlice.LEFT))
	    return new AliceDistanceState(el.getX(),el.getY(),(el.getOrientation()+3)%4,this); 
	return null; 
    }// etatSuivant
